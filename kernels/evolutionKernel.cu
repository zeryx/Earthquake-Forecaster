...
__global__ void evolutionKern(kernelArray<double> vect, kernelArray<int> params, int device_offset){
    ...
    if(vect.array[yourFitness] >= 1){
        ...
        while(1){//select child
            randEng.discard(vect.array[yourFitness] + idx);
        ...
        for(int i=0; i<params.array[1]; i++){//for each weight, lets determine how the weights of the child are altered.
            ...
            atomicExch((unsigned long long*)&vect.array[child_wt +i*ind], __double_as_longlong(result));
        }
